...
describe CustomersController do
  ...
  describe "history" do
    ...
    it "returns an Array" do
      get customer_history_path(id)
      body = JSON.parse(response.body)
      expect(body).must_be_kind_of Array
    end

    it "responds with not found given invalid id" do
      get customer_history_path(-1)
      body = JSON.parse(response.body)

      must_respond_with :not_found
    end

    it "responds with not found if customer has no rental history" do
      get customer_history_path(customers(:three).id)
      body = JSON.parse(response.body)

      must_respond_with :not_found
    end

    it "returns all of the inactive rentals" do
      get customer_history_path(id)
      body = JSON.parse(response.body)

      rentals_length = Rental.where("rentals.active = ? and rentals.customer_id = ?", false, id).count
      expect(body.length).must_equal rentals_length
    end
    ...
    it "can sort rentals based on params" do
      ...
      keys.each do |value|
        get customer_history_path(id), params: {"sort" => value}
    ...
    it "can limit the rentals per page" do
      get customer_history_path(id), params: {"p" => 1, "n" => 2}
